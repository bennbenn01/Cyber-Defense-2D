...
public partial class IdsTower : StaticBody2D
{
	private PackedScene projectile = GD.Load<PackedScene>("res://Sprite/projectile/ids_projectile.tscn");
	private int beamDamage = 4;
	...
	private void FireProjectile()
	{
		...
		IdsProjectile proj = (IdsProjectile)projectile.Instantiate();
		proj.TargetNode = currentTarget;
		proj.projectileDamage = beamDamage;

		Marker2D aim = GetNode<Marker2D>("Aim");
		proj.GlobalPosition = aim.GlobalPosition;
		proj.LookAt(currentTarget.GlobalPosition);

		GetNode("IDSProjectileContainer").CallDeferred("add_child", proj);
	...
	{
		Sprite2D sprite = GetNode<Sprite2D>("IDSTower");
	...
	private bool IsCloseToTower(Vector2 mousePos)
	{
		...
		{
			if (child is IdsTower other && other != this)
			{
