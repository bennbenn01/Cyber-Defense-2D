...
public partial class AntiVirusTower : StaticBody2D
{
	private PackedScene beam = GD.Load<PackedScene>("res://Sprite/projectile/anti_virus_beam.tscn");
	...
	private void FireProjectile()
	{
		...
		AntiVirusBeam proj = (AntiVirusBeam)beam.Instantiate();
		...
		GetNode("AntiVirusBeamContainer").CallDeferred("add_child", proj);
	...
	{
		Sprite2D sprite = GetNode<Sprite2D>("AntiVirusTower");
	...
	private bool IsCloseToTower(Vector2 mousePos)
	{
		...
		{
			if (child is AntiVirusTower other && other != this)
			{
