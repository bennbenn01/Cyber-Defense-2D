...
{
	private PackedScene towerScene = GD.Load<PackedScene>("res://Sprite/tower/ids_tower.tscn");
    private PackedScene previewScene = GD.Load<PackedScene>("res://Sprite/tower/ids_tower_preview.tscn");
    ...
    private const int TOWER_PRICE = 65;
    ...
    private void StartDragTower()
    {
		...
		preview = towersNode.GetNodeOrNull<StaticBody2D>("IDSTowerPreview");
		if (preview == null)
		{
			preview = previewScene.Instantiate<StaticBody2D>();
			preview.Name = "IDSTowerPreview";
			towersNode.AddChild(preview);
		}		
		
		var sprite = preview.GetNode<Sprite2D>("IDSTower");
    ...
    private bool TowerPlacement()
    {
		...
		IdsTower tower = towerScene.Instantiate<IdsTower>();
